...
import { Clock, ArrowRight, User, BookOpen } from "lucide-react";
  ...
  return (
    <section className="py-24 bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        {posts.length > 0 && (
          <div className="flex flex-wrap justify-center gap-6 max-w-5xl mx-auto [&>*]:w-full [&>*]:md:w-[calc(33.333%-1rem)]">
            {posts.map((post, i) => (
                ...
                >
                  <article className="group rounded-2xl border border-border bg-card overflow-hidden hover:-translate-y-2 hover:shadow-xl transition-all duration-500 h-full flex flex-col">
                    <div className="relative h-44 overflow-hidden bg-primary/5 flex items-center justify-center">
                      ...
                      <span className="absolute top-3 left-3 px-3 py-1 text-xs font-semibold rounded-full bg-secondary text-foreground">
                        {post.category}
                      </span>
                    </div>
